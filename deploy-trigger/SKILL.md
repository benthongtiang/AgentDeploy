...
3. Capture all streamed build and startup logs in memory / agent session (no need to save/write log files to disk).
4. If an error is detected in build/runtime logs, exit with code 1, outputting the error snippet/log content clearly so it can be passed to the healing agent.
